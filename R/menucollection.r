...
workspace_management<-function(previous.name=''){
	...
	mergeme<-function(mname,previous.name){
		...
		if(!is.null(new.name)){
            ...
            # both are matrices/datasets
            if((lM==0)&(lN==0)){
                if((rM==rN)&(cM!=cN))M<-cbind(M,N)
                if((rM!=rN)&(cM==cN))M<-rbind(M,N)
                if((rM==rN)&(cM==cN)){
                    oyn<-messageyesno('Merge vertically 1?')
                    oyn<-as.logical(unlist(oyn))
                    if(oyn)M<-rbind(M,N)
                    if(!oyn)M<-cbind(M,N)
                }
            ...
            # M is a vector N a matrix
            if((lM!=0)&(lN==0)){
                if((lM==rN)&(lM!=cN))M<-cbind(M,N)
                if((lM!=rN)&(lM==cN))M<-rbind(M,N)
                if((lM==rN)&(lM==cN)){
                    oyn<-messageyesno('Merge vertically 2?')
                    oyn<-as.logical(unlist(oyn))
                    if(oyn)M<-cbind(M,N)
                    if(!oyn)M<-rbind(M,N)
                }
                if((lM!=rN)&(lM!=cN)){
                    ans<-messagebox('Incompatible Dimensions!')
                    return()
                }
            }
            # N is a vector M a matrix
            if((lN!=0)&(lM==0)){
                if((lN==rM)&(lN!=cM))M<-cbind(M,N)
                if((lN!=rM)&(lN==cM))M<-rbind(M,N)
                if((lN==rM)&(lN==cM)){
                    oyn<-messageyesno('Merge vertically 3?')
                    oyn<-as.logical(unlist(oyn))
                    if(oyn)M<-cbind(M,N)
                    if(!oyn)M<-rbind(M,N)
                }
            ...
            # both N and M are vectors
            if((lN!=0)&(lM!=0)){
                if(lN==lM){
                    oyn<-messageyesno('Merge vertically 4?')
                    oyn<-as.logical(unlist(oyn))
                    if(oyn)M<-cbind(M,N)
                    if(!oyn)M<-rbind(M,N)
                }else{
